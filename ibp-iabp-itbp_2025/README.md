...
**MDE (Mean Distance Error)** -- average violation of distance bounds: $$\mathrm{MDE}(G,X) = \frac{1}{|E|} \sum_{\{v_i, v_j\} \in E} \max\{0,\ \underline{d}_{i,j}-\lVert x_i-x_j\rVert,\ \lVert x_i-x_j\rVert-\bar{d}_{i,j}\}.$$  
