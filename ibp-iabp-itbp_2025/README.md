...
These computational results concern interval distance constraints of varying widths, defined only for pairs of hydrogen atoms. Each interval is modeled as $\mathcal{D}_{ij} = [\underline{d}_{ij}, \overline{d}_{ij}]$, centered at the reference distance and truncated by a van der Waals lower bound and by the NMR cutoff of $5~\text{\AA}$. We distinguish **short-range intervals**, governed by $\varepsilon_{\mathrm{short}}$, for atom pairs belonging to the same or adjacent residues, from **long-range intervals**, governed by $\varepsilon_{\mathrm{long}}$, for all other pairs. This distinction reflects the fact that local contacts are typically characterized by smaller uncertainty, whereas nonlocal contacts require wider intervals.
