...
for sample_dir in "$dir"/sample_size=*; do
	...
	for method_dir in "$sample_dir"/*; do
		...
		for pdb in "${pdbIDs[@]}"; do
			...
			# %s %5d % 11.4f %16d %12d %3d %8.6f %8.6f %8.6f
			printf "\\\\hline\n" >> "$out"
			{
				# 1) %s (PDB ID)
				printf "%s & " "$pdb"
				# 2) %5d (l.e.v.)
				printf "%s & " "$(fmt_int "$lev_num" 5)"
				# 3) % 11.4f (CPU time, leading space for positives)
				printf "%s & " "$(fmt_float_leadspace "$cpu_num" 11 4)"
				# 4) %16d (n.o.e.v.)
				printf "%s & " "$(fmt_int "$noev_num" 16)"
				# 5) %12d (n.o.f.s.)
				printf "%s & " "$(fmt_int "$nofs_num" 12)"
				# 6) %3d (n.o.c.s.)
				printf "%s & " "$(fmt_int "$nocs_num" 3)"
				# 7) %8.6f (maxMDE)
				printf "%s & " "$(fmt_float "$mMDE_num" 8 6)"
				# 8) %8.6f (maxLDE)
				printf "%s & " "$(fmt_float "$mLDE_num" 8 6)"
				# 9) %8.6f (minRMSD)
				printf "%s \\\\\\\\ \n" "$(fmt_float "$minR_num" 8 6)"
			} >> "$out"
		done
